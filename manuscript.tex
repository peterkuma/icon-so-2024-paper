...
\small
$^\mathsf{1}$Department of Meteorology (MISU) and the the Bolin Centre for Climate Research, Stockholm University, Stockholm SE-106 91, Sweden\\[0.2cm]
...
Among them is a 5-km Icosahedral Nonhydrostatic Weather and Climate Model
...
dominated by low clouds, which cannot be observed properly from space due to
...
lifting condensation level being too high.  The reanalyses strongly
...
accurate in all, but ICON is too unstable over low-latitude SO and too humid in
the boundary layer. MERRA-2 is too humid at all heights. SO cloud biases are a
substantial issue in the GSRM, but it is an improvement over the
