...
2010--2013 (inclusive) and \textbf{(b)} ICON in model years 2021--2023 (free
running) as identified by CyTRACK, expressed as a fraction of the number of
days with cyclonic activity, defined as grid points located within a double
radius of any cyclone on a given day (UTC).
}
\label{fig:lts}
\end{figure}
...
legend, and the relative frequency of occurrence (RFO) of the subset. The bands
span the 16$^\mathrm{th}$--84$^\mathrm{th}$ percentile calculated from the set
of all voyages and stations. The subsets \textbf{(a--g)} are described in the
text (Section \ref{sec:results}).
}
