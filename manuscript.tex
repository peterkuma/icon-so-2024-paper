...
global climate models are in development as the forthcoming phase of climate
modelling. One such model is a 5-km version of the Icosahedral Nonhydrostatic
Weather and Climate Model (ICON) developed jointly by Deutscher Wetterdienst
(DWD) and the Max-Planck-Institute for Meteorology (MPI-M). Because of the high
resolution, most parametrisations, such as that of convection and clouds, can
be avoided.
...
observations conducted with ceilometers and lidars on board of Research Vessel
(RV) \textit{Polarstern} and other voyages. Altogether, we analyse over 2000
days (about 6 years) of data from 31 voyages and 1 sub-antarctic station
...
will conventionally refer to the SO as ocean regions south of 40$^\circ$S,
low-latitude SO as 40--55$^\circ$S and high-latitude SO as south of
55$^\circ$S.
...
over the entire 40--60$^\circ$S SO.
...
increasing by up to 1$^\circ$C.  \cite{luo2023} identified that the SO SST bias
...
\begin{figure}
\centering
\includegraphics[width=\textwidth]{img/map-fig.png}
\caption{
\textbf{(a)} A map showing the tracks of 31 voyages of RV \emph{Polarstern}, RSV
\emph{Aurora Australis}, RV \emph{Tangaroa}, RV \emph{Nathaniel B. Palmer}, and
HMNZS \emph{Wellington} and one sub-Antarctic station (Macquarie Island)
...
The dashed line at 40$^\circ$S delineates the Southern Ocean area of our
analysis. A photo of \textbf{(b)} RV \emph{Polarstern} (© Folke
Mehrtens/Alfred-Wegener-Institut), \textbf{(c)} Lufft CHM 15k installed on RV
\emph{Tangaroa} (© Peter Kuma), \textbf{(d)} Vaisala CL51 install at the
Macquarie Island station (© Jeﬀ Aquilina, Bureau of Meteorology).
...
\caption{
An example of attenuated volume backscattering coefficient (AVBC) measured by
CL51 during 24 hours on the PS81/3 voyage and an equivalent AVBC simulated with
the ALCF from ERA5 data during the same time period. Shown is also the cloud
mask determined by the ALCF.
}
\label{fig:example}
\end{figure}
...
\begin{figure}
\centering
\includegraphics[width=\textwidth]{img/lts-dist.pdf}
\caption{
...
2010 and 2021 (Fig. \ref{fig:map} and Table \ref{tab:voyages}). Altogether, the
voyages and the station comprised 2421 days of data south of 40$^\circ$S, but
the availability of ceilometer data was smaller.
...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{img/cloud_occurrence_agg.pdf}
\caption{
Cloud occurrence by height aggregated from the 31 voyages and one station for
observations (OBS), the ICON model, and the MERRA-2 and ERA5 reanalyses. The
total cloud fraction (CF) is shown in the legend. The ranges are from the
16$^\mathrm{th}$ to the 84$^\mathrm{th}$ percentile. \textbf{(a)} all conditions,
\textbf{(b)}, \textbf{(c)} cyclonic and non-cyclonic conditions as identified
by cyclone tracking, \textbf{(d)}, \textbf{(e)} stable and unstable conditions
as divided by the lower tropospheric stability criterion.
}
